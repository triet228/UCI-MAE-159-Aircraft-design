...
% C_L loop constants
Conventional_airfoil = 1; % 0 for supercritical airfoil
Velocity_approach = 140; % knot
Mach_cruise = 0.82;
Range = 6000; % Nmi
max_percent_fuel_at_landing = 75/100;
...
for i = 1:AR_number_of_steps
    Direct_operating_cost_per_ton_mile = 0;
    ...
    while fail == 1
        ...
        Range_all_out = 1E10;
        if AR < 8
            Range_iteration_limit = 2000;
        else
            Range_iteration_limit = 1000;
        end
        Range_iteration = 0;
        while (abs(Range_all_out-Range_all_out_guess) > 150) && Range_iteration < Range_iteration_limit
            ...
            while abs(C_L_final-C_L) > .005
                ...
                t_c = Mach_div_to_thickness_to_chord_ratio(Conventional_airfoil, Swept_angle, Mach_div);
        ...
        %% Gradient_check
        Gradient_check(Number_of_engine, Gradient_1, Gradient_2, Gradient_3, Gradient_approach, Gradient_landing)
...
function thickness_to_chord_ratio = Mach_div_to_thickness_to_chord_ratio(Conventional_airfoil, Swept_angle, Mach_div)
...
if Conventional_airfoil == 1
    if (Swept_angle >= 0) && (Swept_angle <= 10)
        thickness_to_chord_ratio = ((Swept_angle - 0) / (10 - 0)) * (t_c_10 - t_c_0) + t_c_0;
    elseif (Swept_angle >= 10) && (Swept_angle < 15)
        thickness_to_chord_ratio = ((Swept_angle - 10) / (15 - 10)) * (t_c_15 - t_c_10) + t_c_10;
    elseif (Swept_angle >= 15) && (Swept_angle < 20)
        thickness_to_chord_ratio = ((Swept_angle - 15) / (20 - 15)) * (t_c_20 - t_c_15) + t_c_15;
    elseif (Swept_angle >= 20) && (Swept_angle < 25)
        thickness_to_chord_ratio = ((Swept_angle - 20) / (25 - 20)) * (t_c_25 - t_c_20) + t_c_20;
    elseif (Swept_angle >= 25) && (Swept_angle < 30)
        thickness_to_chord_ratio = ((Swept_angle - 25) / (30 - 25)) * (t_c_30 - t_c_25) + t_c_25;
    elseif (Swept_angle >= 30) && (Swept_angle < 35)
        thickness_to_chord_ratio = ((Swept_angle - 30) / (35 - 30)) * (t_c_35 - t_c_30) + t_c_30;
    elseif (Swept_angle >= 35) && (Swept_angle <= 40)
        thickness_to_chord_ratio = ((Swept_angle - 35) / (40 - 35)) * (t_c_40 - t_c_35) + t_c_35;
    end
elseif Conventional_airfoil == 0
    fprintf("For developer: need to digitize supercritical plot.\n")
else
...
function Gradient_check(Number_of_engine, Gradient_1, Gradient_2, Gradient_3, Gradient_approach, Gradient_landing)
if Number_of_engine == 2
    if (Gradient_1 < 0)
